...
import { LoginImg,} from '../../../../public/assests/images'
...
const Login:FC  = ()=> {
  ...
  return (
    <React.Fragment>
      <Navbar/>
      <ContentBox sx={{my:6}}>
      <Box overflow={"hidden"} sx={{py:3,boxShadow: "0 12px 40px rgba(0,0,0,0.2)",}} position={"relative"} width={"100%"} maxWidth={"1300px"} m={"auto"} borderRadius={3} border={"2px solid var(--primary)"}>
      ...
      <Text1 sx={{fontSize:"16px",textAlign:"center",mx:"auto", bgcolor:"rgba(223, 175, 73, 0.15)",width:"fit-content",px:1.5,py:0.5,border:"1px solid rgba(223, 175, 73, 0.3)",color:"var(--text)",borderRadius:"12px"}}>Create New Account! <span onClick={()=>{router.push(Route.SIGNUP)}} style={{color:"var(--primary)",cursor:"pointer"}}>SignUp</span></Text1>  
      <Grid container alignItems={"center"} mt={4} px={{xs:2,sm:4,md:6}}>
         <Grid size={{xs:12,md:6}} display={"flex"} justifyContent={"center"}>
          <motion.div initial={{ x: 200, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 1, ease: "easeOut" }}>
           <Image src={LoginImg} alt="img_here"/>
          </motion.div>
           </Grid>
         <Grid size={{xs:12,md:6}} display={"flex"} flexDirection={"column"} maxWidth={{xs:"100%",sm:"70%",md:"450px"}}> 
          <motion.div style={{width:"100%"}} initial={{ x: -200, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 1, ease: "easeOut" }}>
            <H6 sx={{fontFamily:"var(--font-heading)",}} >Log in to Continue</H6>
            <TextArea type='email' placeholder='JohnDoe@gmail.com' label='Enter Email :' mt={2}/>
            <TextArea type='password' placeholder='Your Password' label='Enter Password :' mt={2}/>
            <OutlineButton sx={{mt:5,width:"100%"}}>Log In</OutlineButton>
